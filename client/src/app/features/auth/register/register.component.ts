  ...
  template: `
    <div class="min-h-[80vh] flex items-center justify-center">
      <div class="card w-full max-w-md">
        ...
        <p class="text-slate-400 mb-6 text-sm">Empezá a trackear tu portfolio argentino</p>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
          ...
          <div>
            ...
            <input type="password" formControlName="password" class="input" placeholder="Mínimo 6 caracteres" />
          </div>
          <button type="submit"
                  class="btn-primary w-full"
                  [disabled]="form.invalid || (loading$ | async)">
            @if (loading$ | async) { Creando cuenta... } @else { Crear cuenta }
          </button>
        </form>

        <p class="text-center text-slate-400 text-sm mt-6">
          ¿Ya tenés cuenta?
          <a routerLink="/auth/login" class="text-blue-400 hover:text-blue-300">Ingresá</a>
        </p>
      </div>
    </div>
  `
