...
using DolarEnVivo.Application.UseCases.Portfolio.Commands.AddPosition;
using DolarEnVivo.Application.UseCases.Portfolio.Commands.DeletePosition;
using DolarEnVivo.Application.UseCases.Portfolio.Commands.UpdatePosition;
using DolarEnVivo.Application.UseCases.Portfolio.Queries.GetPositions;
using MediatR;
...
public class PortfolioController : BaseController
{
    ...
    public async Task<IActionResult> GetPositionsAsync(CancellationToken cancellationToken) =>
        Ok(
            await this.Mediator.Send(
                new GetPositionsQuery { UserId = this.CurrentUserId },
                cancellationToken
            )
        );
    ...
    {
        await this.Mediator.Send(
            new DeletePositionCommand { Id = id, UserId = this.CurrentUserId },
            cancellationToken
        );
        return NoContent();
    }
